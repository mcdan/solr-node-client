...
var http = require('http'),
   ...
   SolrCloud = require('./solrcloudconfig');
...
function Client(options){
   console.trace('Creating client: %j', options);
   this.options = {
      ...
      zkHost : options.zkHostString || null
   };
   ...
   if (this.options.zkHost) {
      this.solrCloudConfig = new SolrCloud(this.options.zkHost, this.options.core);
   } else {
      this.solrCloudConfig = null;
   }
}
...
Client.prototype.update = function(data,options,callback){
   ...
   var self = this;
   if ((this.solrCloudConfig !== null) && (!this.solrCloudConfig.isReady())) {
      console.trace('need to wait for Cloud config to load.');
      this.solrCloudConfig.once('ready', function() {
         var t = self.solrCloudConfig.getServer();
         self.options.host = t.hostname;
         self.options.port = t.port;
         self.options.path = t.path;
         self.update(data, options, callback);
      });
      return;
   }
...
Client.prototype.get = function(handler,query,callback){
   ...
   if ((this.solrCloudConfig !== null) && (!this.solrCloudConfig.isReady())) {
      ...
      this.solrCloudConfig.once('ready', function() {
         var t = self.solrCloudConfig.getServer();
         console.trace('Server: %j', t);
         self.options.host = t.hostname;
         self.options.port = t.port;
         self.options.path = t.path;
         self.get(handler, query, callback);
      });
      return;
   }
