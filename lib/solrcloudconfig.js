...
function parseStateJSON(client, details, emitThisEvent) {
...
function loadCollectionStatus(client, emitThisEvent, path) {
    'use strict';
    var collectionStatePath = path || '/collections/' + client.options.collection + '/state.json';
    if (traceLogging) {
    ...
    client.zkClient.getData(collectionStatePath, 
        ...
        function(error, data) {
            ...
            if (error) {
                if (traceLogging) {
                    console.trace("Could not get Collection state from inside collection, trying root clusterstate: " + error + "\n" + error.stack);
                }
                loadCollectionStatus(self, emitThisEvent, '/clusterstate.json');
                return;
            }
            if (traceLogging) {
            ...
            parseStateJSON(self, details, emitThisEvent);
        }
    );
}
